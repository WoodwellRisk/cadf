...
const ForecastLayer = ({
  ...
  useEffect(() => {
    ...
    if (!map.getLayer(layerId)) {
      map.addLayer({
        ...
        paint: {
          'circle-color': [
            'case',
            ['<', ['get', band], thresholds[0]],
            colormap[0],
            ['all', ['>=', ['get', band], thresholds[0]], ['<', ['get', band], thresholds[1]]],
            colormap[1],
            ['all', ['>=', ['get', band], thresholds[1]], ['<', ['get', band], thresholds[2]]],
            colormap[2],
            ['all', ['>=', ['get', band], thresholds[2]], ['<', ['get', band], thresholds[3]]],
            colormap[3],
            ['all', ['>=', ['get', band], thresholds[3]], ['<', ['get', band], thresholds[4]]],
            colormap[4],
            ['all', ['>=', ['get', band], thresholds[4]], ['<', ['get', band], thresholds[5]]],
            colormap[5],
            ['all', ['>=', ['get', band], thresholds[5]], ['<', ['get', band], thresholds[6]]],
            colormap[6],
            ['all', ['>=', ['get', band], thresholds[6]], ['<', ['get', band], thresholds[7]]],
            colormap[7],
            ['all', ['>=', ['get', band], thresholds[7]], ['<', ['get', band], thresholds[8]]],
            colormap[8],
            ['all', ['>=', ['get', band], thresholds[8]], ['<', ['get', band], thresholds[9]]],
            colormap[9],
    ...
    map.on('click', layerIdRef.current, (event) => {
      ...
      // could do the same thing with a bounding box: https://docs.mapbox.com/mapbox-gl-js/example/using-box-queryrenderedfeatures/
  ...
  useEffect(() => {
    ...
    updatePaintProperty(map, layerId, 'fill-opacity', opacity);
  }, [opacity]);
  ...
  useEffect(() => {
    if (zoom < zoomChange) {
      ...
      if (zoom < zoomChange) {
        updatePaintProperty(map, layerId, 'circle-stroke-width', 0.25);
      } else if (zoom < 5) {
        updatePaintProperty(map, layerId, 'circle-stroke-width', 0.5);
      } else if (zoom < 5.5) {
        updatePaintProperty(map, layerId, 'circle-stroke-width', 1.0);
      } else {
        updatePaintProperty(map, layerId, 'circle-stroke-width', 1.5);
      }
    }
  }, [zoom]);
